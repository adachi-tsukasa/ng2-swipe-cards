...
export class TinderCardDirective implements OnInit, OnDestroy {
  ...
  onReleaseLikeCb(event: any) {
    ...
    if (this._overlay) {
      let overlayElm = <HTMLElement>(
        this.element.querySelector(".tinder-overlay")
      );
  ...
  onSwipeLikeCb(event: any) {
    if (this._overlay) {
      ...
      let opacity =
        ((event.distance < 0 ? event.distance * -1 : event.distance) * 0.5) /
  ...
  onSwipe(event: any) {
    let like =
      (this.orientation === "y" && event.deltaY < 0) ||
    ...
    if (!!this._overlay) {
      ...
      this.renderer.setStyle(
        this.overlayElement,
        "background-color",
        this._overlay[like ? "like" : "dislike"].backgroundColor
      );
    }
    this.translate({
      x: event.deltaX,
  ...
  translate(params: any) {
    if (!this.fixed) {
      this.renderer.setStyle(
        this.element,
        "transition",
        "transform " + (params.time || 0) + "s ease"
      );
      this.renderer.setStyle(
        this.element,
        "webkitTransform",
        "translate3d(" +
          (params.x &&
          (!this.orientation || this.orientation.indexOf("x") != -1)
            ? params.x
            : 0) +
          "px, " +
          (params.y &&
          (!this.orientation || this.orientation.indexOf("y") != -1)
            ? params.y
            : 0) +
          "px, 0) rotate(" +
          params.rotate +
  ...
  initCallLike() {
    this._callLike.subscribe((params: any) => {
      ...
      this.renderer.setStyle(
        this.overlayElement,
        "background-color",
        this._overlay[params.like ? "like" : "dislike"].backgroundColor
      );
      this.destroy(200);
    });
  }
