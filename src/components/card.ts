...
export class CardComponent implements OnInit, OnDestroy {
  ...
  translate(params: any) {
    if (!this.fixed && !this.released) {
      ...
      this.renderer.setStyle(
        ...
        "translate3d(" +
          (params.x &&
          (!this._orientation || this._orientation.indexOf("x") != -1)
            ? params.x
            : 0) +
          "px, " +
          (params.y &&
          (!this._orientation || this._orientation.indexOf("y") != -1)
            ? params.y
            : 0) +
          "px, 0) rotate(" +
          (params.rotate || 0) +
  ...
  onPanEnd(event: any) {
    if (!this.fixed && !this.released) {
      if (
        (this._orientation == "x" &&
          (this.releaseRadius.x < event.deltaX ||
            this.releaseRadius.x * -1 > event.deltaX)) ||
        (this._orientation == "y" &&
          (this.releaseRadius.y < event.deltaY ||
            this.releaseRadius.y * -1 > event.deltaY)) ||
        this.releaseRadius.x < event.deltaX ||
        this.releaseRadius.x * -1 > event.deltaX ||
        this.releaseRadius.y < event.deltaY ||
        this.releaseRadius.y * -1 > event.deltaY
